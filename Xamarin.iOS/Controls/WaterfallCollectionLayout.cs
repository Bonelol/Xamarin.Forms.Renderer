...
namespace MySIT.Mobile.iOS.Controls
{
    ...
    public class WaterfallCollectionLayout : UICollectionViewLayout
    {
        ...
        private nfloat _minimumColumnSpacing = 4;
        private nfloat _minimumInterItemSpacing = 4;
        ...
        private readonly Dictionary<nint, UICollectionViewLayoutAttributes> _headersAttributes = new Dictionary<nint, UICollectionViewLayoutAttributes>();
        private readonly Dictionary<nint, UICollectionViewLayoutAttributes> _footersAttributes = new Dictionary<nint, UICollectionViewLayoutAttributes>();
        private readonly List<CGRect> _unionRects = new List<CGRect>();
        private readonly List<nfloat> _columnHeights = new List<nfloat>();
        ...
        private readonly nfloat _unionSize = 10;
        #endregion
        ...
        public WaterfallCollectionLayout(NSCoder coder)
            : base(coder)
        {
        ...
        public override void PrepareLayout()
        {
            ...
            for (nint section = 0; section < numberofSections; ++section)
            {
                ...
                var minimumInterItemSpacing = MinimumInterItemSpacingForSection?.Invoke(CollectionView, this, section) ?? MinimumColumnSpacing;
                // Calculate widths
                var width = CollectionView.Bounds.Width - SectionInset.Left - SectionInset.Right;
                var itemWidth = (nfloat)Math.Floor((width - ((ColumnCount - 1) * MinimumColumnSpacing)) / ColumnCount);

                // Calculate section header
                var headerHeight = HeightForHeader?.Invoke(CollectionView, this, section) ?? HeaderHeight;

                UICollectionViewLayoutAttributes attributes;
                if (headerHeight > 0)
                {
                    attributes = UICollectionViewLayoutAttributes.CreateForSupplementaryView(UICollectionElementKindSection.Header, NSIndexPath.FromRowSection(0, section));
                    attributes.Frame = new CGRect(0, top, CollectionView.Bounds.Width, headerHeight);
                ...
                for (nint n = 0; n < itemCount; n++)
                {
                    ...
                    var itemSize = SizeForItem?.Invoke(CollectionView, this, indexPath) ?? new CGSize(0, 0);
                ...
                footerHeight = HeightForFooter?.Invoke(CollectionView, this, section) ?? FooterHeight;
        ...
        public override UICollectionViewLayoutAttributes[] LayoutAttributesForElementsInRect(CGRect rect)
        {
            ...
            return _allItemAttributes.Where(attr => rect.IntersectsWith(attr.Frame)).ToArray();
        }
